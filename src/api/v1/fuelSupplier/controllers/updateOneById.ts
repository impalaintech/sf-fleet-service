...
import { z } from "zod";
import fuelSupplierService from "@/lib/fuelSupplier";
...
const updateById = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const validatedData = updateFuelSupplierDTOSchema.parse(convertedData);

    //get single item with validated id
    const data = await fuelSupplierService.getSingleById(validatedId);
    ...
    const updated = await fuelSupplierService.updateById(
      validatedId,
      validatedData
    );
    ...
    //get single item with validated id
    const finalData = await fuelSupplierService.getSingleById(validatedId);
